...
module.exports = {
  ...
  getFreeToPlayGames: (req, res, next) => {
    db.query(
      "SELECT game.name,coverH,thumbnail,website,store_catalog.seller as seller,store_catalog.launcher as launcher,s.name as sellerName, l.name as launcherName,url,region,price, game.date " +
  ...
  getMostRecentsGames: (req, res, next) => {
    db.query(
      "SELECT game.name,coverH,thumbnail,website,store_catalog.seller as seller,store_catalog.launcher as launcher,s.name as sellerName, l.name as launcherName,url,region,ROUND(price / 60, 2) as price, game.date FROM game" +
      ...
      " ORDER BY game.date DESC LIMIT 20",
      function (err, data, fields) {
        if (err) return next(new AppError(err));
        res.status(200).json({
          data,
  ...
  getGamesUnderPrice: (req, res, next) => {
    db.query(
      "SELECT game.name,coverH,thumbnail,website,store_catalog.seller as seller,store_catalog.launcher as launcher,s.name as sellerName, l.name as launcherName,url,region,ROUND(price / 60, 2) as price_game, game.date" +
      " FROM game JOIN store_catalog ON game.id=store_catalog.game" +
      " JOIN store_price ON game.id=store_price.game" +
      " AND store_catalog.seller = store_price.seller" +
      " AND store_catalog.launcher = store_price.launcher" +
